...
namespace AngularJS_CS.Models
{
	...
	public class Dal : IDal
	{
		...
		public Doc_Web ObtenirDoc(string name, Groupe group)
		{
			if(group == null)
			{
				return bdd.Doc_Web.FirstOrDefault(doc => doc.nom == name);
			}
			else
			{
				return bdd.Doc_Web.FirstOrDefault(doc => doc.nom == name && (doc.Groupe.Any(gr => gr.Id == group.Id) || doc.Groupe == null || doc.Groupe.First() != null));
			}
		}
